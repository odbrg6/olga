...
from ..helpers.utils import _format, _olgautils, parse_pre, reply_id
...
async def video_olgafile(event):  # sourcery no-metrics
    reply = await event.get_reply_message()
    args = event.pattern_match.group(1)
    olgaid = await reply_id(event)
    ...
    flag = True
    olgaevent = await edit_or_reply(event, "- يتم التحويل انتظر قليلا")
    olgafile = await reply.download_media(file="./temp/")
    if mediatype in ["Gif", "Video", "Sticker"]:
        if not olgafile.endswith((".webp")):
            if olgafile.endswith((".tgs")):
                await Convert.to_gif(event, olgafile, file="circle.mp4", noedits=True)
                olgafile = "./temp/circle.mp4"
            media_info = MediaInfo.parse(olgafile)
            aspect_ratio = 1
            ...
            if aspect_ratio != 1:
                ...
                await _olgautils.runcmd(
                    f'ffmpeg -i {olgafile} -vf "crop={crop_by}:{crop_by}" {PATH}'
                )
            else:
                copyfile(olgafile, PATH)
            if str(olgafile) != str(PATH):
                os.remove(olgafile)
            try:
                olgathumb = await reply.download_media(thumb=-1)
        ...
        try:
            ...
            olgathumb = os.path.join("./temp", "thumb.jpg")
            await thumb_from_audio(olgafile, olgathumb)
        if olgathumb is not None and not os.path.exists(olgathumb):
            olgathumb = os.path.join("./temp", "thumb.jpg")
            copyfile(thumb_loc, olgathumb)
        if (
            olgathumb is not None
            and not os.path.exists(olgathumb)
            and os.path.exists(thumb_loc)
        ):
            flag = False
            olgathumb = os.path.join("./temp", "thumb.jpg")
            copyfile(thumb_loc, olgathumb)
        if olgathumb is not None and os.path.exists(olgathumb):
            await _olgautils.runcmd(
                f"""ffmpeg -loop 1 -i {olgathumb} -i {olgafile} -c:v libx264 -tune stillimage -c:a aac -b:a 192k -vf \"scale=\'iw-mod (iw,2)\':\'ih-mod(ih,2)\',format=yuv420p\" -shortest -movflags +faststart {PATH}"""
            )
            os.remove(olgafile)
        else:
            os.remove(olgafile)
            return await edit_delete(
                olgaevent, "**- يجب ان يكون المقطع الصوتي يحتوي على خللفية**", 5
            )
    if mediatype in [
        "Voice",
        "Audio",
        ...
        "Video",
        "Sticker",
    ] and not olgafile.endswith((".webp")):
        if os.path.exists(PATH):
            ...
            uploaded = await event.client.fast_upload_file(
                ...
                progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                    progress(d, t, olgaevent, c_time, "Uploading....")
                ),
            )
            ul.close()
            media = types.InputMediaUploadedDocument(
                ...
                force_file=False,
                thumb=await event.client.upload_file(olgathumb)
                if olgathumb
                else None,
            )
    ...
    img.save(im)
    im.seek(0)
    await event.client.send_file(event.chat_id, im, reply_to=olgaid)
    await olgaevent.delete()
...
async def on_file_to_photo(event):
    ...
    if image.size > 10 * 1024 * 1024:
        return
    olgat = await edit_or_reply(event, "**- جار التحويل انتظر قليلا**")
...
async def _(event):
    olgareply = await event.get_reply_message()
    memetype = await meme_type(olgareply)
    if memetype == "Gif":
    ...
    reply_to_id = await reply_id(event)
    olgafile = await event.client.download_media(olgareply)
    final = await Convert.to_gif(event, olgafile, file="animation.mp4", noedits=True)
    olgagif = final[1]
    if olgagif is None:
        return await edit_delete(
            olgaevent, "**- لا يمكنني تحويل هذا النوع من الميديا الى متحركة**"
        )
    sandy = await event.client.send_file(
        event.chat_id,
        olgagif,
