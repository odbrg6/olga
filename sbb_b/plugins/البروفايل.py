...
async def _(event):
    ...
    try:
        ...
        await olgaevent.edit(str(e))
    else:
        if photo:
            await olgaevent.edit("**- جار الرفع على اتلجرام انتظر قليلا**")
            if photo.endswith((".mp4", ".MP4")):
                size = os.stat(photo).st_size
                if size > 2097152:
                    await olgaevent.edit("**- يجب ان يكون الحجم اقل من 2 ميغا**")
                    os.remove(photo)
                    return
                olgapic = None
                olgavideo = await event.client.upload_file(photo)
            else:
                olgapic = await event.client.upload_file(photo)
                olgavideo = None
            try:
                await event.client(
                    functions.photos.UploadProfilePhotoRequest(
                        file=olgapic, video=olgavideo, video_start_ts=0.01
                    )
                )
            except Exception as e:
                await olgaevent.edit(f"**خطا:**\n`{e}`")
            else:
                await edit_or_reply(olgaevent, "**- تم بنجاح تحديث صورة الحساب**")
    try:
        os.remove(photo)
    except Exception as e:
        LOGS.info(str(e))
...
async def count(event):
    ...
    b = 0
    result = ""
    olgaevent = await edit_or_reply(event, "**- جار التعداد انتظر قليلا**")
