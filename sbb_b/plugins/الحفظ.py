...
async def _(event):
    ...
    if not reply_message.media:
        ...
        return
    chat = "@VS_Robot"
    olgaevent = await edit_or_reply(event, "**- انتظر قليلا الان من فضلك**")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            if response1.text:
                ...
                return await olgaevent.edit(response1.text, parse_mode=parse_pre)
            await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
        ...
        if not input_str:
            await edit_or_reply(olgaevent, response4.text)
