...
def verify_cond(olgaarray, text):
    return any(i in text for i in olgaarray)


def pack_name(userid, pack, is_anim, is_video):
    if is_anim:
        return f"olga_{userid}_{pack}_anim"
    if is_video:
        return f"olga_{userid}_{pack}_vid"
    return f"olga_{userid}_{pack}"


def char_is_emoji(character):
    return character in olgaemoji.UNICODE_EMOJI["en"]
...
async def delpack(olgaevent, conv, args, packname):
    try:
...
async def kang(args):
    ...
    if message and message.media:
        ...
        if memetype == "Photo":
            olgaevent = await edit_or_reply(args, f"`{random.choice(KANGING_STR)}`")
            photo = io.BytesIO()
            ...
            if memetype == "Video Sticker":
                ...
                for attribute in attributes:
                    if isinstance(attribute, DocumentAttributeSticker):
                        if message.media.document.size > 261120:
                            ...
                            await edit_or_reply(
                                olgaevent, f"`{random.choice(KANGING_STR)}`"
                            )
                        else:
                            olgaevent = await edit_or_reply(
                ...
                await edit_or_reply(olgaevent, f"`{random.choice(KANGING_STR)}`")
        else:
    ...
    if photo:
        ...
        if len(splat) == 2:
            if char_is_emoji(splat[0][0]):
                if char_is_emoji(splat[1][0]):
                    return await olgaevent.edit("**- عليك التأكد من اوامر الملصقات**")
        ...
        if (
            ...
            await olgaevent.edit("**- جار جلب حزمة ثانية انتظر قليلا**")
            async with args.client.conversation("@Stickers") as conv:
...
async def pack_kang(event):
    ...
    olga = base64.b64decode("VHdIUHd6RlpkYkNJR1duTg==")
    if (
        not reply
    ...
    for message in reqd_sticker_set.documents:
        if "image" in message.mime_type.split("/"):
            ...
            await edit_delete(olgaevent, "- هذه الصيغة غير صالحة")
            return
        if photo:
            ...
            if (
                ...
                async with event.client.conversation("@Stickers") as conv:
                    pack, olgapackname = await newpacksticker(
                        olgaevent,
                        conv,
                        cmd,
                        event,
                ...
                async with event.client.conversation("@Stickers") as conv:
                    pack, olgapackname = await add_to_pack(
                        olgaevent,
                        conv,
                        event,
                        packname,
            ...
            if olgapackname is None:
                return
            if olgapackname not in blablapacks:
                blablapacks.append(olgapackname)
    ...
    await olgaevent.edit(result)


@sbb_b.ar_cmd(pattern="فاس$")
async def pussyolga(event):
    ...
    packname = f"olga_{userid}_temp_pack"
    response = urllib.request.urlopen(
        urllib.request.Request(f"http://t.me/addstickers/{packname}")
    )
    htmlstr = response.read().decode("utf8").split("\n")
...
async def pic2packcmd(event):
    ...
    if (
        mediatype == "Sticker"
        and reply.document.mime_type == "appliolgaion/x-tgsticker"
    ):
        return await edit_delete(
            event,
    ...
    olgaevent = await edit_or_reply(event, "**- جار التحويل الان انتظر قليلا**")
    try:
    ...
    image = await Convert.to_image(
        olgaevent, reply, dirct="./temp", file="stickers.png", noedits=True
    )
    if image[1] is None:
        return await edit_delete(
    ...
    newimg.save(new_img)
    new_img.seek(0)
    olgaevent = await event.edit("**- جار صنع حزمة انتظر قليلا**")
