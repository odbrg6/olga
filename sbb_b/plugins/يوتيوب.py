    ...
    for url in urls:
        ...
        for _path in glob.glob(os.path.join(Config.TEMP_DIR, str(startTime), "*")):
            if _path.lower().endswith((".jpg", ".png", ".webp")):
                ...
                _fpath = _path
        if not _fpath:
            return await edit_delete(olgaevent, "**❃ غير قادر على الرفع**")
...
async def download_video(event):
    ...
    reply_to_id = await reply_id(event)
    for url in urls:
        ytdl_data = await ytdl_down(olgaevent, video_opts, url)
        if ytdl_down is None:
            return
        try:
            f = pathlib.Path("olga_ytv.mp4")
            print(f)
            olgathumb = pathlib.Path("olga_ytv.jpg")
            if not os.path.exists(olgathumb):
                olgathumb = pathlib.Path("olga_ytv.webp")
            if not os.path.exists(olgathumb):
                olgathumb = None
            await olgaevent.edit(
                f"**⋗ جار رفع الفيديو :**\
...
async def insta_dl(event):
    ...
    v2 = "@videomaniacbot"
    media_list = []
    olgaevent = await edit_or_reply(event, "**↣ جار التحميل انتظر قليلا**")
    async with event.client.conversation(v1) as conv:
        ...
        await edit_or_reply(olgaevent, "**↣ جار التحميل من مصدر اخر**")
