...
async def fileinfo(file):
    ...
    olga_json = json.loads(x)["media"]["track"]
    dic = {
        "path": file,
        "size": int(olga_json[0]["FileSize"]),
        "extension": olga_json[0]["FileExtension"],
        "type": "None",
        "format": "None",
        "audio": "None",
    }
    with contextlib.suppress(IndexError, KeyError):
        dic["format"] = olga_json[0]["Format"]
        dic["type"] = olga_json[1]["@type"]
        if "ImageCount" not in olga_json[0]:
            dic["duration"] = int(float(olga_json[0]["Duration"]))
            dic["bitrate"] = int(olga_json[0]["OverallBitRate"]) // 1000
        dic["height"] = int(olga_json[1]["Height"])
        dic["width"] = int(olga_json[1]["Width"])
        dic["audio"] = "Present" if olga_json[0]["AudioCount"] else "None"
    return dic
