...
async def hide_inlinebot(borg, bot_name, text, chat_id, reply_to_id, c_lick=0):
    ...
    olga = await sticcers[c_lick].click("me", hide_via=True)
    if olga:
        await borg.send_file(int(chat_id), olga, reply_to=reply_to_id)
        await olga.delete()


async def make_inline(text, borg, chat_id, reply_to_id):
    olgainput = f"Inline buttons {text}"
    results = await borg.inline_query(Config.TG_BOT_USERNAME, olgainput)
...
def higlighted_text(
    ...
    for pic_no, list_text in enumerate(texts):
        ...
        output_img = f"./temp/olga{pic_no}.jpg"
...
async def waifutxt(text, chat_id, reply_to_id, bot):
    ...
    olga = await sticcers[0].click("me", hide_via=True)
    if olga:
        await bot.send_file(int(chat_id), olga, reply_to=reply_to_id)
        await olga.delete()
