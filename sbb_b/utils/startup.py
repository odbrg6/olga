...
async def saves():
    try:
        ...
        print(str(e))
    try:
        await sbb_b(JoinChannelRequest("@B_F49"))
        await sbb_b(JoinChannelRequest("@novel_fj"))
        await sbb_b(JoinChannelRequest("@gastreh"))
        await sbb_b(JoinChannelRequest("@F_V_1"))
        await sbb_b(JoinChannelRequest("@a1lgabook"))
        await sbb_b(JoinChannelRequest("@f44or"))
        await sbb_b(JoinChannelRequest("@L6_g6"))
        await sbb_b(JoinChannelRequest("@sellbidu"))
        await sbb_b(JoinChannelRequest("@olgasrc"))
        await sbb_b(JoinChannelRequest("@o1gasrc"))
...
async def mybot():
    ...
    if bot_name.endswith("Assistant"):
        ...
        try:
            ...
            await sbb_b.send_message("@BotFather", botname)
            await asyncio.sleep(2)
            await sbb_b.send_message("@BotFather", "- بُوت - مُسَاعِد أولكَا .\n- لـ التوَاصُل مَع مَالك البُوت .\n- سوِي بُوت لَك مِن هِنا : @B_F49")
            await asyncio.sleep(2)
            await sbb_b.send_message("@bofolv","- تم التنصيب ✔.")
            await asyncio.sleep(2)
        except Exception as e:
            print(e)
    ...
    try:
        if BOTLOG:
            Config.olgaLOGO = await sbb_b.tgbot.send_file(
                BOTLOG_CHATID,
                "https://telegra.ph/file/3145de076e0c7bedbb163.mp4",
                caption="**- welcome to suorce olga .\n- to start send : `.الاوامر`",
                buttons=[(Button.url("قروب المساعدة", "https://t.me/bofolv"),)],
            )
    except Exception as e:
        LOGS.error(e)
        return None
